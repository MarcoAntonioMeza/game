...
def caminos_nvl_1(img,img_vida):
    ...
    vida.add(Corazon(SCREEN_WIDTH//2,nvl2-tam,80,80,img_vida))
    #vida.add(Corazon(SCREEN_WIDTH//4,nvl2-tam,80,80,img_vida))
    for i in range(100,SCREEN_WIDTH,300):
        #vida.add(Corazon(i+100,nvl2-tam,80,80,img_vida))
        zombie.add(Enemy(i+140, nvl2-nvl, scale=scale, speed=speed))
    
    vida.add(Corazon(SCREEN_WIDTH//2,nvl1-tam,80,80,img_vida))
    vida.add(Corazon(SCREEN_WIDTH//2,nvl1-tam,80,80,img_vida))
    for i in range(50,SCREEN_WIDTH,200):
        zombie.add(Enemy(i+120, nvl1-nvl, scale=scale, speed=speed))
        #vida.add(Corazon(i+520,nvl1-tam,80,80,img_vida))
...
def caminos_nvl_3(img,img_vida):
    ...
    zombie.add(Enemy(SCREEN_WIDTH//2, nvl2-nvl, scale=scale, speed=speed+.4))
    zombie.add(Enemy(SCREEN_WIDTH//6, nvl2-nvl, scale=scale, speed=speed+.4))
    ...
    zombie.add(Enemy(SCREEN_WIDTH//2, nvl1-nvl, scale=scale, speed=speed+0.3))   
    zombie.add(Enemy(SCREEN_WIDTH//5, nvl1-nvl, scale=scale, speed=speed+0.3))   
        
    
    return star,zombie,vida
